...
uint16_t LSM9DS1::begin(uint8_t agAddress, uint8_t mAddress, i2c_inst_t *i2c_instance)
{
	...
	settings.device.i2c = i2c_instance;
...
void LSM9DS1::xgWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		i2cWriteByteToRegister(i2c_default, _xgAddress, subAddress, data);
...
void LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		i2cWriteByteToRegister(i2c_default, _mAddress, subAddress, data);
	// else if (settings.device.commInterface == IMU_MODE_SPI)
